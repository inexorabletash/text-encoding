...
(function(global) {
   ...
   * @return {Object}
   */
  function ToDictionary(o) { return Object(o); }
  ...
  /** @const */ var end_of_stream = -1;

  /**
   * @constructor
   * @param {Array.<number>} tokens Array of tokens that provide the stream.
   */
  ...
  Encoder.prototype = {
     ...
     * @return {number} The last byte emitted.
     */
    encode: function(output_byte_stream, stream, code_point) {}
  };
  ...
  /**
   * @param {number} pointer The |pointer| to search for in the shift_jis index.
   * @return {?number} The code point corresponding to |pointer| in |index|,
  ...
  function TextDecoder(opt_encoding, options) {
    ...
    if (this._encoding === null || this._encoding.name === 'replacement')
      throw RangeError('Unknown encoding: ' + opt_encoding);
  ...
  TextDecoder.prototype = {
    ...
    decode: function decode(opt_view, options) {
      ...
      while (!input_stream.endOfStream()) {
        bite = input_stream.read();
        code_point = /** @type {?number} */ (this._decoder.decode(input_stream, bite));
        if (code_point !== null && code_point !== finished)
      ...
      if (!this._streaming) {
        do {
          ...
          if (code_point !== null && code_point !== finished)
            output_stream.emit(code_point);
        } while (code_point !== finished && !input_stream.endOfStream());
        this._decoder = null;
      }
  ...
  TextEncoder.prototype = {
    ...
    encode: function encode(opt_string, options) {
      ...
      while (!input_stream.endOfStream()) {
        this._encoder.encode(output_stream, input_stream, input_stream.read());
      }
      if (!this._streaming) {
        /** @type {number} */
        var last_byte;
        do {
          last_byte = this._encoder.encode(output_stream, input_stream, input_stream.read());
        } while (last_byte !== end_of_stream);
        this._encoder = null;
      }
  ...
  function UTF8Decoder(options) {
     ...
     *     data exists in the input stream to decode a complete code point.
     */
    this.decode = function(stream, bite) {
  ...
  function UTF8Encoder(options) {
     ...
     * @return {number} The last byte emitted.
     */
    this.encode = function(output_byte_stream, stream, code_point) {
      /** @type {number} */
    ...
    this.decode = function(stream, bite) {
      ...
      if (shiftjis_lead !== 0x00) {
        ...
        if (inRange(bite, 0x40, 0x7E) || inRange(bite, 0x80, 0xFC))
          var pointer = (lead - lead_offset) * 188 + bite - offset;
        ...
        // pointer − 8836.
        if (code_point === null && inRange(pointer, 8836, 10528))
