...
test("UTF-8 encoding", function() {
  ...
  for (i = 0; i < 0x10FFFF; i += BATCH_SIZE) {
    ...
    stringEncoding.encode(str, actual.buffer, 0, 'UTF-8');

    arrayEqual(expected, actual, 'expected equal encodings');
  }
});

test("UTF-8 decoding", function() {
  expect(1088);
  ...
  for (i = 0; i < 0x10FFFF; i += BATCH_SIZE) {
    ...
    actual = stringEncoding.decode(new Uint8Array(encoded).buffer, 0, encoded.length, 'UTF-8');
...
test("Encode-decode", function() {
  ...
  // FIXME: acceptable if other endian-ness, too
  var expected_utf16 = [0xff, 0xfe, 0x7A, 0x00, 0xA2, 0x00, 0x34, 0x6C, 0x34, 0xD8, 0x1E, 0xDD, 0xFF, 0xDB, 0xFD, 0xDF];
  ...
  function check(string, encoding, expected) {
    ...
    stringEncoding.encode(string, array.buffer, 0, encoding);
    arrayEqual(expected, array, 'expected equal encodings ' + encoding);

    var decoded = stringEncoding.decode(new Uint8Array(expected).buffer, 0, expected.length, encoding);
...
test("Encoding names are case insensitive", function() {
  var encodings = [
...
test("Byte-order marks", function() {
  expect(5);
  ...
  equal(string, stringEncoding.decode(new Uint8Array(utf8).buffer, 0, utf8.length, 'UTF-8'));
  equal(string, stringEncoding.decode(new Uint8Array(utf16le).buffer, 0, utf16le.length, 'UTF-16LE'));
  equal(string, stringEncoding.decode(new Uint8Array(utf16be).buffer, 0, utf16be.length, 'UTF-16BE'));

  raises(function () { stringEncoding.decode(new Uint8Array(utf16le).buffer, 0, utf16le.length, 'UTF-16BE'); });
  raises(function () { stringEncoding.decode(new Uint8Array(utf16be).buffer, 0, utf16be.length, 'UTF-16LE'); });
});

test("Null termination", function() {
  expect(6);
  ...
  encodings.forEach(
    function(test) {
      ...
      var array = new Uint8Array(len);
      stringEncoding.encode(with_null, array.buffer, 0, test.encoding);

      var found_len = stringEncoding.stringLength(array.buffer, 0, array.byteLength, test.encoding);
      var decoded = stringEncoding.decode(array.buffer, 0, found_len, test.encoding);
